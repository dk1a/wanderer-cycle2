...
} from "../mud/hooks/useChangeCycleEquipment";
import {
  EquipmentSlot,
  useEquipmentSlots,
} from "../mud/hooks/useEquipmentSlots";
...
export const InventoryProvider = (props: { children: ReactNode }) => {
  ...
  const equipmentListWithSlots = useMemo(() => {
    return sortedEquipmentList.map((data): EquipmentData => {
      ...
      const equippedToSlot = equipmentSlots.find(
        ({ equipped }) => data.entity === equipped?.entity
      );

      const availableSlots = (() => {
        if (equippedToSlot) return;
        const equippableToSlots = equipmentSlots.filter(
  ...
  const presentProtoEntityIds = useMemo(() => {
    ...
    const presentProtoEntityIds = new Set(
      equipmentList.map(({ protoEntityId }) => protoEntityId)
    );
    // the filter just uses the sorting order of `equipmentProtoEntityIds`
    return equipmentProtoEntityIds.filter((protoEntityId) =>
      presentProtoEntityIds.has(protoEntityId)
    );
  }, [equipmentList]);
