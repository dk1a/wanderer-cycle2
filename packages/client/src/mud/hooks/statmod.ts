...
function sumForOp(statmods: ReturnType<typeof useTopicStatmods>, opFilter: Op) {
  return statmods
    .filter(({ statmodPrototype: { op } }) => op === opFilter)
...
function sumForElementalOp(
  statmods: ReturnType<typeof useTopicStatmods>,
  opFilter: Op,
  elementFilter: StatmodElement,
) {
  return statmods
    .filter(
      ({ statmodPrototype: { op, element } }) =>
        op === opFilter && element === elementFilter,
    )
    .reduce((acc, curr) => acc + curr.value, 0);
}

export const useGetValuesFinal = (
  ...
  return useMemo(() => {
    ...
    const resultMul = Math.floor(
      (resultBadd * (100 + sumForOp(statmods, Op.MUL))) / 100,
    );
...
export const useGetValuesElementalFinal = (
  ...
  return useMemo(() => {
    ...
    for (const element of statmodElements) {
      const resultBadd =
        baseValues[element] + sumForElementalOp(statmods, Op.BADD, element);
      const resultMul = Math.floor(
        (resultBadd * (100 + sumForElementalOp(statmods, Op.MUL, element))) /
          100,
      );
      const resultAdd =
        resultMul + sumForElementalOp(statmods, Op.ADD, element);
...
export const useTopicStatmods = (
  ...
  const topicEntityId = useMemo(
    () => keccak256(toUtf8Bytes(topic)) as EntityID,
    [topic],
  );
  ...
  const scope = useMemo(() => {
    ...
    return defaultAbiCoder.encode(
      ["uint256", "uint256"],
      [world.entities[targetEntity], topicEntityId],
    );
  }, [world, targetEntity, topicEntityId]);

  const appliedEntities = useEntityQuery(
    [HasValue(StatmodScope, { value: scope }), Has(StatmodValue)],
    true,
  );

  return useMemo(() => {
    return appliedEntities.map((appliedEntity: Entity) => {
      const statmodValue = getComponentValueStrict(StatmodValue, appliedEntity);
      const fromPrototype = getComponentValueStrict(
        FromPrototype,
        appliedEntity,
      );
      ...
      if (!protoEntity) {
        throw new Error(
          "statmod appliedEntities conversion: protoEntity is absent from world",
        );
      }
      const statmodPrototype = getStatmodPrototype(
        world,
