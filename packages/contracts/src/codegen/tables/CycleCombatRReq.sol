...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("CycleCombatRReq")))
);
ResourceId constant CycleCombatRReqTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0028030220040400000000000000000000000000000000000000000000000000
);
...
library CycleCombatRReq {
  ...
  /**
   * @notice Get the length of winnerPStat.
   */
  function lengthWinnerPStat(bytes32 requestId) internal pure returns (uint256) {
    return 3;
  }
   ...
   * @notice Get the length of winnerPStat.
   */
  function _lengthWinnerPStat(bytes32 requestId) internal pure returns (uint256) {
    return 3;
  }
  ...
  /**
   * @notice Get the length of loserPStat.
   */
  function lengthLoserPStat(bytes32 requestId) internal pure returns (uint256) {
    return 3;
  }
   ...
   * @notice Get the length of loserPStat.
   */
  function _lengthLoserPStat(bytes32 requestId) internal pure returns (uint256) {
    return 3;
  }
  ...
  function set(
    ...
    PackedCounter _encodedLengths = encodeLengths(winnerPStat, loserPStat);
  ...
  function set(bytes32 requestId, CycleCombatRReqData memory _table) internal {
    ...
    PackedCounter _encodedLengths = encodeLengths(_table.winnerPStat, _table.loserPStat);
  ...
  function decodeStatic(
    ...
    mapEntity = (Bytes.slice32(_blob, 0));

    connection = (uint32(Bytes.slice4(_blob, 32)));

    fortune = (uint32(Bytes.slice4(_blob, 36)));
  }
  ...
  function encodeLengths(
    ...
    unchecked {
      _encodedLengths = PackedCounterLib.pack(winnerPStat.length * 4, loserPStat.length * 4);
    }
  }
